...
<svelte:head>
    ...
    <meta name="keywords" content="{[...(skills?.map(s => s.title) || []), ...(services?.map(s => s.title) || []), profile?.seo_keywords, 'portfolio', 'digital solutions', profile?.full_name].filter(Boolean).join(', ')}">
            ...
            <div class="max-w-6xl mx-auto">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    {#each services as service, i}
                        <div use:scrollReveal={{ delay: i * 100 }} class="group relative bg-slate-900 border border-slate-800 p-6 lg:p-8 rounded-[2rem] transition-all duration-500 hover:border-sky-500/30 shadow-xl overflow-hidden flex flex-col reveal">
                            <!-- Header Area -->
                            <div class="mb-6">

                                <h3 class="text-xl lg:text-2xl font-bold text-white group-hover:text-sky-400 transition-colors tracking-tighter mb-2">
                                    {service.title}
                                </h3>
                            ...
                            <div class="mt-auto relative z-20">
                                <a 
                                    href={'https://wa.me/' + formattedPhone + '?text=' + encodeURIComponent('Halo, saya mau konsultasi ' + service.title)}
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    class="w-full flex items-center justify-center gap-2 bg-slate-900 border border-[#25D366]/30 text-[#25D366] px-5 py-3 rounded-xl font-bold transition-all duration-300 group/btn hover:bg-[#25D366] hover:text-white hover:border-[#25D366] hover:shadow-[0_0_20px_rgba(37,211,102,0.6)] active:scale-95"
                                >
                ...
                {#each paginatedProjects as project, i}
                    <a use:scrollReveal={{ delay: (i % 3) * 100 }} href="/project/{project.slug}" class="group relative flex flex-col bg-slate-900 border border-slate-800 rounded-3xl hover:border-sky-500/50 transition-all duration-500 shadow-xl overflow-hidden reveal">
                        ...
                        <div class="p-6 lg:p-8 flex-1 flex flex-col">
                            ...
                            <div class="flex items-center gap-2 text-sky-500 font-bold text-sm uppercase tracking-wider group-hover:gap-4 transition-all group-hover:text-sky-400">
    ...
    <footer id="contact" class="pt-32 pb-16 bg-slate-950/80 border-t border-slate-900/50 relative overflow-hidden">
        ...
        <div class="container mx-auto px-6 relative z-10">
            <div class="flex flex-col lg:flex-row justify-between gap-16 lg:gap-8 pt-20 border-t border-slate-900">
                ...
                <!-- Legal Links -->
                <div use:scrollReveal={{ delay: 150 }} class="min-w-[200px] reveal">
                    <h4 class="text-white font-black uppercase tracking-widest text-sm mb-8">Kebijakan</h4>
                    <ul class="space-y-4">
                        <li>
                            <a href="{baseUrl}/terms" class="text-slate-200 hover:text-sky-400 transition-colors text-sm font-medium">Syarat & Ketentuan</a>
                        </li>
                        <li>
                            <a href="{baseUrl}/privacy" class="text-slate-200 hover:text-sky-400 transition-colors text-sm font-medium">Kebijakan Privasi</a>
                        </li>
                        <li>
                            <a href="{baseUrl}/refund" class="text-slate-200 hover:text-sky-400 transition-colors text-sm font-medium">Kebijakan Pengembalian Dana</a>
                        </li>
                    </ul>
                </div>
